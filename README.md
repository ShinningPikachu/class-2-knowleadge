...
- **Library** — use a desktop-style two-pane file explorer with a folder sidebar and icon grid, drag local files directly onto the subject root or any folder to upload them, drag existing file tiles into any folder to move them, recognize file types from simple PDF/audio/Markdown/PowerPoint icons, click files to preview their contents, double-click a file to open its full-page view in a new browser tab, remove or rename files and folders, and search extracted text from the magnifier field at the top. A lecture recording opens with its cleaned, synchronized transcript timeline; class slides and concise notes are linked as one lecture set.
...
Start in **Library** to create a subject and optional lecture folders. The file explorer has persistent folder locations on the left, file/folder icons in the center, and a content preview on the right—there are no repeated document rows. Drag local files onto the subject root or any folder in the explorer to upload them; drop into open space to upload into the folder being viewed. Click a file once to preview it, double-click a file to open its full-page view in a new tab, or press and drag a file tile onto any folder (including the subject root) to move it. Use the magnifier field at the top to search this subject. A lecture recording appears above its cleaned transcript; playback highlights the matching timestamped passage, and selecting a passage seeks the audio. When a lecture PDF/PPT is clicked, the same preview pane shows the selected slide on top and its exact slide-plus-professor summary below, with arrow navigation and a **🧠** action for that slide. The recording, class slides, and concise notes PDF are linked as one lecture set. In **Lecture Notes**, provide one or both lecture inputs, choose a priority and optional destination subject, and select **Queue Lecture Task**. A supplied title is kept verbatim for the task and its folder, which is created immediately; a blank title is inferred automatically into a consistent name such as `Lecture_01_Introduction`. Within each lecture folder, learner-facing files use simple names such as `Slides.pdf`, `Recording.m4a`, and `Notes.pdf`. With both inputs, recording transcription and slide extraction run together before professor speech is aligned to the relevant slides.
